...
async function main() {
  ...
  try {
    ...
    for (const block of subjects) {
      const subjectResult = await client.query("SELECT id FROM subjects WHERE name = $1", [block.subject]);
      if (subjectResult.rowCount === 0) {
        throw new Error(`Materia "${block.subject}" nu există în tabela subjects. Adaug-o întâi (vezi database/init.sql).`);
      }
